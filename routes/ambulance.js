...
// Ambulance Login
router.post('/ambulancelogin', (req, res, next) => {
  passport.authenticate('ambulancereglocal', (err, user) => {
    if (err) {
      return next(err);
    }
    if (!user) {
      return res.redirect('/ambulancelogin');
    }
  ...
  AmbulanceRegistration.register(
    ...
    (err, item) => {
      if (err) {
        ...
        data.NODE_ENV = process.env.NODE_ENV;
        data.user = req.user;
        return res.render('ambulanceregistration', { data });
      }
