...
router.post('/patientlogin', (req, res, next) => {
  passport.authenticate('patientlocal', (err, user) => {
    if (err) {
      return next(err);
    }
    if (!user) {
...
router.post('/patientregistration', (req, res) => {
  ...
  Patient.register(newpatient, req.body.password, (err, item) => {
    if (err) {
      ...
      data.user = req.user;
      data.NODE_ENV = process.env.NODE_ENV;
      return res.render('patientregistration', { data });
    }
    console.log(item);
  ...
  (req, res) => {
    ...
    Promise.all([FindAmbulancePromise])
      .then((result) => {
        ...
        const emailmessage = `HI you have a new ambulance request from ${req.user.name}.
        Patient Details: \nName: ${req.user.name}\n
        ...
        if (email && password) {
          ...
          transporter.sendMail(mailOptions, (error, info) => {
            if (error) {
              console.log('Error sending ambulance notification email:', error);
            } else {
              console.log('Ambulance notification email sent', info);
            }
          });
        } else {
          console.log('SENDOR_EMAIL or SENDOR_PASSWORD not set; skipping ambulance notification email.');
        }
